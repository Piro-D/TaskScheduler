...
# Load local .env values when present; Azure App Service provides the same keys as App Settings.
load_dotenv()
...
# Azure deployments must set FLASK_SECRET_KEY in App Settings.
# The fallback is only for local imports and should not be used in production.
...
ACTIVE_SCHEDULE_FILE = RUNTIME_DIR / "active_schedule.json"  # Legacy fallback; state_service.py uses state.json.
...
# Allow insecure OAuth redirects only during explicit local development.
