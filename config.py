...
# External service credentials loaded from the local .env file.
GROQ_API_KEY = os.getenv("GROQ_API_KEY")
CLIENT_ID = os.getenv("GOOGLE_CLIENT_ID")
CLIENT_SECRET = os.getenv("GOOGLE_CLIENT_SECRET")

# Google Calendar OAuth scopes required by the scheduler.
SCOPES = [
...
# Local runtime files generated by preprocessing, scheduling, and feedback flows.
...
# Defaults used when a user has not customized scheduling preferences.
...
# Bounds for form inputs that control focus-session and break lengths.
...
if os.getenv("FLASK_ENV") == "development":
    # Google OAuth allows HTTP redirect URIs only when this local development flag is set.
